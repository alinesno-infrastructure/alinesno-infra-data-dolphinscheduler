...
<template>
  <div class="timing-process-model">
    ...
    <div class="clearfix list">
      <el-button type="primary"  style="margin-left:20px" size="small" :loading="spinnerLoading" @click="preview()">{{$t('Execute time')}}</el-button>
      <div class="text">
        {{$t('Timing')}}
      </div>
      ...
      <div class="cont">
        <el-select
          ...
          <el-input slot="trigger" readonly slot-scope="{ selectedModel }" :value="selectedModel ? selectedModel.label : ''" style="width: 200px;" @on-click-icon.stop="warningGroupId = {}">
            ...
            <em slot="suffix" class="el-icon-bottom" style="font-size: 13px;" v-show="!warningGroupId.id"></em>
          </el-input>
          <el-option
    ...
    <div class="submit">
      ...
      <el-button type="primary" size="small" :loading="spinnerLoading" @click="ok()">{{spinnerLoading ? $t('Loading...') : (timingData.item.crontab ? $t('Edit') : $t('Create'))}} </el-button>
    </div>
  </div>
</template>
<script>
